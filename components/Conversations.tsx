  ...
  const fetchConversations = async () => {
    try {
      const response = await axios.get(
        `/api/conversations?email=${session?.user?.email}`
      );
    ...
    <div className="my-4 w-[45] bg-base-200 border-white border-xl">
      ...
      >
        <ListItem
          ...
          onPointerEnterCapture={() => console.log("pointer")}
          onPointerLeaveCapture={() => console.log("pointer")}
          className="mb-6 border-gray-100 border-1"
        >
          <button
            onClick={handleAddConversation}
            className="w-full text-base-content flex flex-row items-center justify-between border-1 border-base-content"
          >
